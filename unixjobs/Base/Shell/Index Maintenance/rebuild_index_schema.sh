...
#-------------------------------------------------------
# File: rebuild_index_schema.sh
# Desc: Rebuild Indexes - Single Schema
# Runs: Noon Every Sunday = 0 12 * * 0
# Parm: 1 - Schema for which to Rebuild Indexes
...
if [ $# -lt 1 ]
then
  echo "Usage: `basename $0` SCHEMA {PARALLEL_DEGREE (default 4)}"
  exit 1
fi

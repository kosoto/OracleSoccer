...
#-------------------------------------------------------
# File: deallocate_index_tablespace.sh
# Desc: Perform Index Deallocate - Single Tablespace
# Runs: Four Times per Month = 0 0 1,8,15,22 * *
# Parm: 1 - Tablespace for which to Deallocate Index Space
...
if [ $# -lt 1 ]
then
  echo "Usage: `basename $0` TABLESPACE {MEGS_KEEP (default 4)}"
  exit 1
fi
